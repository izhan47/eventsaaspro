...
class SettingsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        if (!$setting->exists) {
            $setting->fill(["display_name"=> "Site Name", "value"=> "EventSaaSPro", "details"=> null, "type"=> "text", "order"=> "1", "group"=> "Site", ])->save();
        }
        ...
        if (!$setting->exists) {
            $setting->fill(["display_name"=> "Meta Title", "value"=> "EventSaaSPro", "details"=> null, "type"=> "text", "order"=> "5", "group"=> "SEO", ])->save();
        }

        $setting=$this->findSetting("seo.meta_description");
        if (!$setting->exists) {
            $setting->fill(["display_name"=> "Meta Description", "value"=> "eventsaaspro", "details"=> null, "type"=> "text", "order"=> "7", "group"=> "SEO", ])->save();
        }
        ...
        if (!$setting->exists) {
            $setting->fill(["display_name"=> "Email", "value"=> "info@eventsaaspro.com", "details"=> null, "type"=> "text", "order"=> "14", "group"=> "Contact", ])->save();
        }
        ...
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Admin Title", "value" => "EventSaaSPro", "details"=> null, "type" => "text", "order" => "24", "group" => "Admin", ])->save();
        }

        $setting = $this->findSetting("admin.description");
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Admin Description", "value" => "EventSaaSPro Admin Panel", "details"=> null, "type" => "text", "order" => "25", "group" => "Admin", ])->save();
        }
        ...
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Mail Sender Email", "value" => "eventsaaspro@classiebit.com", "details"=> null, "type" => "text", "order" => "42", "group" => "Mail", ])->save();
        }

        $setting = $this->findSetting("mail.mail_sender_name");
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Mail Sender Name", "value" => "EventSaaSPro", "details"=> null, "type" => "text", "order" => "43", "group" => "Mail", ])->save();
        }
       ...
       $setting = $this->findSetting("apps.usaepay_production_mode");
        if (!$setting->exists) {
            ...
            ]), "type" => "checkbox", "order" => "36", "group" => "Apps", ])->save();
        }

        $setting = $this->findSetting("apps.usaepay_sourceKey");
        if (!$setting->exists) {
            $setting->fill(["display_name" => "USAePay Source Key", "value" => "", "details"=> null, "type" => "text", "order" => "36", "group" => "Apps", ])->save();
        }

        $setting = $this->findSetting("apps.usaepay_pin");
        if (!$setting->exists) {
        ...
        if (!$setting->exists) {
            ...
            $setting->fill(["display_name" => "Stripe Public Key", "value" => "", "details"=> null, "type" => "text", "order" => "37", "group" => "Apps", ])->save();
        }

        $setting = $this->findSetting("apps.stripe_secret_key");
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Stripe Secret Key", "value" => "", "details"=> null, "type" => "text", "order" => "37", "group" => "Apps", ])->save();
        }
        $setting = $this->findSetting("apps.stripe_comic_book_fee_account_id");
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Stripe Account Id For ComicBook Fee", "value" => "", "details"=> null, "type" => "text", "order" => "37", "group" => "Apps", ])->save();
        }
    }
