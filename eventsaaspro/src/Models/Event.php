...
class Event extends Model
{
    ...
    protected $with = ['bookings', 'comedian', 'user','coupon'];
    ...
    {
        return $this->hasMany(ComedianEvent::class);
    }
    ...
    public function get_related_events($event)
    {
        return Event::select('events.*')
            ...
            ->selectRaw("(SELECT CT.name FROM categories CT WHERE CT.id = events.category_id) category_name")
            ->selectRaw("(SELECT SD.repetitive_type FROM schedules SD WHERE SD.event_id = events.id limit 1) repetitive_type")
            ->take(4) // Limit the result to the first 4 events
            ->get();
    }
    ...
    public function events($params = [])
    {
        ...
        $query
            ->where(["events.publish" => 1, "categories.status" => 1])
        ...
        return $query->without('bookings')->orderBy('events.start_date', 'ASC')->paginate(9);
    }
    ...
    {
        $query = Event::leftJoin("categories", "categories.id", '=', "events.category_id")
            ->select(["events.*", "categories.name as category_name"])
            ->selectRaw("(SELECT CN.country_name FROM countries CN WHERE CN.id = events.country_id) country_name")
            ->selectRaw("(SELECT SD.repetitive_type  FROM schedules SD WHERE SD.event_id = events.id limit 1 ) repetitive_type")
            ->where(['events.publish' => 1, 'events.status' => 1, 'categories.status' => 1])
            ->orderBy('start_date')->without('bookings');
        ...
        return $query->orderBy('events.start_date', 'ASC')->limit(20)->get();

    }

    // get nearby events
    public function get_events_nearby()
    {
        $query = Event::leftJoin("categories", "categories.id", '=', "events.category_id")
            ->select(["events.*", "categories.name as category_name"])
            ->selectRaw("(SELECT CN.country_name FROM countries CN WHERE CN.id = events.country_id) country_name")
        ...
        return $query->orderBy('events.start_date', 'ASC')->limit(12)->get();
    ...
    public function get_organizers($search = null)
    {
        ...
        $query->select('name', 'id', 'email', 'avatar')
            ->where('role_id', 3);
        if (!empty($search)) {
            $query
