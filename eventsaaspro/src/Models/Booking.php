...
use Eventsaaspro\Scopes\BulkScope;
use DB;
...
class Booking extends Model
{
    ...
    protected static function booted()
    {
        ...
        if (!empty(\Request::route())) {
            if (\Request::route()->getName() != 'voyager.bookings.bulk_bookings' && \Request::route()->getName() != 'eventsaaspro.get_booking' && \Request::route()->getName() != 'eventsaaspro.verify_ticket') {
    ...
    public function get_organiser_bookings($params = [])
    {
        ...
        // in case of searching by title or customer_email
        if (!empty($params['search'])) {
            $query->where(function ($query) use ($params) {
                $query->where('bookings.event_title', 'like', '%' . $params['search'] . '%')
                    ->orWhere('bookings.customer_email', 'like', '%' . $params['search'] . '%')
                    ->orWhere('bookings.order_number', 'like', '%' . $params['search'] . '%');
            });
        }
