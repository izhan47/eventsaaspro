...
class Commission extends Model
{
    ...
    public function show_commission_organisers_wise($organiser_id = null, $params = [], $refunds = false)
    {
        ...
        $select = array(
            ...
            "$table.transferred",
            "$table.settled",
            "$table.status as commission_status",
            "$table.month_year",
            "$table.event_id",
            "$table.comedian_total_commission",
            DB::raw("(SELECT U.name FROM users U WHERE U.id = org_id) as organiser_name"),
            ...
            DB::raw("(SELECT E.thumbnail FROM events E WHERE E.id = $table.event_id) as event_thumbnail"),
            DB::raw("SUM($table.customer_paid) as customer_paid_total"),
            DB::raw("SUM($table.admin_commission) as admin_commission_total"),
            DB::raw("SUM($table.admin_tax) as admin_tax_total"),
            DB::raw("SUM($table.organiser_earning) as organiser_earning_total"),
            DB::raw("COUNT($table.booking_id) as count_bookings"),
            "$table.updated_at",
            DB::raw("(SELECT E.status FROM events E WHERE E.id = $table.event_id) as event_status"),
        );
        ...
        $query->groupBy(["$table.event_id", "$table.transferred"])
    ...
    public function organiser_total_earning($organiser_id = null)
    {
        return Commission::select([
            ...
            DB::raw("SUM(CASE WHEN events.status = 2 THEN organiser_earning ELSE 0 END) as organiser_earning_total"),
        ])
            ->leftJoin('events', 'commissions.event_id', '=', 'events.id')
            ->where([
