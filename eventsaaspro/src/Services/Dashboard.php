...
use Eventsaaspro\Http\Controllers\Voyager\VoyagerBaseController;
...
class Dashboard extends Controller
{
     ...
     *  index page
     */
    public function index(Request $request, $user_id = null, $view = 'eventsaaspro::vendor.voyager.dashboard')
    {
    ...
    public function export_sales_report(Request $request, $user_id = null)
    {
        ...
        foreach($sales_report as  $key => $item)
        {
            ...
            $csvData[$key][ __('voyager::generic.Checked In')]  = $item['checked_in'] > 0 ? __('eventsaaspro-pro::em.yes') : __('eventsaaspro-pro::em.no') ;
            ...
            $csvData[$key][__('voyager::generic.Payout') ] = ($item['transferred'] <= 0 && $item['organiser_earning'] > 0) ?  __('eventsaaspro-pro::em.pending') : __('eventsaaspro-pro::em.transferred');

        }

        // add extra row
        $csvData[count($sales_report) + 1][__('voyager::generic.Order Number')]    =  null;
        $csvData[count($sales_report) + 1][__('voyager::generic.Event')]    =  __('eventsaaspro-pro::em.total');
