...
use Eventsaaspro\EventmieServiceProvider;
...
class InstallCommand extends Command
{
     ...
     * @var string
     */
    protected $name = 'eventsaaspro-pro:install';
     ...
     * @var string
     */
    protected $description = 'EventSaaSPro Installer';
    ...
    private function install(Filesystem $filesystem)
    {
        ...
        $this->info('1. Publishing EventSaaSPro core assets: config, languages & dummy content');
        $this->call('vendor:publish', ['--provider' => EventmieServiceProvider::class]);

        // 2. Run EventSaaSPro migrations
        $this->info('2. Migrating the EventSaaSPro database tables into your application');
        $this->call('migrate', ['--force' => $this->option('force')]);

        // 3. Extend App\User to EventSaaSPro user model
        $this->info('3. Attempting to set EventSaaSPro User model as parent to App\User');
        if (file_exists(app_path('User.php')) || file_exists(app_path('Models/User.php'))) {
            ...
            if ($str !== false) {
                $str = str_replace('extends Authenticatable', "extends \Eventsaaspro\Models\User", $str);

                file_put_contents($userPath, $str);
            }
        } else {
            $this->warn('Unable to locate "User.php" in app or app/Models.  Did you move this file?');
            $this->warn('You will need to update this manually.  Change "extends Authenticatable" to "extends \Eventsaaspro\Models\User" in your User model');
        }
        ...
        // 4. Add EventSaaSPro Route
        $this->info('4. Adding EventSaaSPro routes to your application routes/web.php');
        $routes_contents = $filesystem->get(base_path('routes/web.php'));
        if (false === strpos($routes_contents, 'EventSaaSPro::routes()')) {
            $filesystem->append(
        ...
        $version = EventSaaSPro::getVersion();
        $this->info("Congrats! EventSaaSPro version $version installed successfully! Best of Luck!");
    }
