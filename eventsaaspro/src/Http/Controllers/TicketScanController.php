...
class TicketScanController extends Controller
{
    ...
    // ticket scan
    public function index(Request $request, $view = 'eventsaaspro::ticket_scan.index', $extra = [])
    {
    ...
    public function get_booking(Request $request)
    {
        ...
        if(empty($booking))
        {
            ...
            return response()->json([
                    'errors' => [
                        'msg' => [__('eventsaaspro-pro::em.ticket').' '.__('eventsaaspro-pro::em.not_found')]
                    ]
            ], 422);
        ...
        if($booking->event_start_date != Carbon::today()->format('Y-m-d'))
        {
            return response()->json([
                    'errors' => [
                        'msg' => [__('eventsaaspro-pro::em.ticket_scan_error')]
                    ]
            ], 422);
        }
        ...
        {
            $msg = __('eventsaaspro-pro::em.already_cheked_in');
        ...
        if(!empty($event->merge_schedule))
        {
            ...
            if(!empty($merge_dates))
            {
                ...
                if(!is_numeric($date_exist)) {
                    $msg = __('eventsaaspro-pro::em.booking').' '.__('eventsaaspro-pro::em.not_found').' OR '.__('eventsaaspro-pro::em.ticket_scan_error');
                    $request->validate([
                        'already_scan' => 'required'
                    ],[
                        'already_scan.required' => $msg
    ...
    public function verify_ticket(Request $request, $organiser_id = null)
    {
        ...
        if($booking->status != 1)
        {
            $msg = __('eventsaaspro-pro::em.disabled_ticket');
            session()->flash('error', $msg);
            return error_redirect($msg);
        }
        ...
        if(!empty($event->merge_schedule))
        {
            ...
            if(!empty($merge_dates))
            {
                ...
                if(!is_numeric($date_exist)) {
                    $msg = __('eventsaaspro-pro::em.booking').' '.__('eventsaaspro-pro::em.not_found');
                    session()->flash('error', $msg);
                    return error_redirect($msg);
                }
        ...
        $url = route('eventsaaspro.ticket_scan');
        $msg = __('eventsaaspro-pro::em.success_cheked_in');
