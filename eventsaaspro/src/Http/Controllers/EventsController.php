...
class EventsController extends Controller
{
     ...
     * @return array
     */
    public function index($view = 'eventsaaspro::events.index', $extra = [])
    {
     ...
     * @return array
     */
    public function show(Event $event, $view = 'eventsaaspro::events.show', $extra = [])
    {
        ...
        $related_events        = $this->event->get_related_events($event);
        ...
        return EventSaaSPro::view($view, compact(
            ...
            'tickets', 'currency', 'booked_tickets', 'total_capacity', 'extra', 'is_usaepay','related_events'));
    }
    ...
    public function tag($event_slug = null, $tag_title = null, $view = 'eventsaaspro::tags.show', $extra = [])
    {
        $tag_title  = str_replace('-', ' ', strtolower(urldecode($tag_title)));
        $tag        = $this->tag->get_tag_by_title($tag_title);

        if(empty($tag))
            return error_redirect(__('eventsaaspro-pro::em.tag').' '.__('eventsaaspro-pro::em.not_found'));

        return EventSaaSPro::view($view, compact( 'tag', 'extra'));
