...
class MyEventsController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->ComedianEvent = new ComedianEvent;
    ...
    public function completeEvent($event_id, $status)
    {
        $total = 0;
        $comedianCommissionTotal = 0;

        try {
            $event = $this->event->where('id', $event_id)->first();
            if ($status == 2) {
                $bookings = $this->booking->where('event_id', $event_id)->where('eventbrite_id', null)->get();
                $totalPersons = 0;
                foreach ($bookings as $key => $value) {
                    $total += $value->price;
                    $totalPersons += Ticket::where('id', $value->ticket_id)->pluck('no_of_persons')->first();
                }
                $comedians = $this->ComedianEvent->where('event_id', $event_id)->get();
                foreach ($comedians as $key => $value) {
                    $comedianTotal = 0;
                    if ($value->fixed == 1 && $value->percent == 0) {
                        $comedianTotal = self::checkComedianThreshold('fixed', $totalPersons, $total, $value);

                    } elseif ($value->percent == 1 && $value->fixed == 0) {
                        $comedianTotal = self::checkComedianThreshold('percent', $totalPersons, $total, $value);
                    } else {
                        $comedianTotal = self::checkComedianThreshold('percent', $totalPersons, $total, $value);
                        $comedianTotal += self::checkComedianThreshold('fixed', $totalPersons, $total, $value);
                    }
                    $value->total = $comedianTotal;
                    $comedianCommissionTotal += $comedianTotal;
                    $value->save();
                }

                $commission = $this->Commission->where('event_id', $event_id)->get();
                foreach ($commission as $key => $value) {
                    $value->comedian_total_commission = $comedianCommissionTotal;
                    $value->save();
                }
            }
            $event->status = $status;
    ...
    public function checkComedianThreshold($type, $totalPersons, $event_total, $comedian)
    {
        if ($type == 'fixed') {

            foreach ($comedian->fixed_value as $key => $value) {
                if ($value['min_person'] == 0 && $value['max_person'] == null) {
                    return $value['amount'];
                } elseif ($value['max_person'] != null && $value['min_person'] <= $totalPersons && $value['max_person'] >= $totalPersons) {
                    return $value['amount'];
                } elseif ($value['max_person'] == null && $value['min_person'] <= $totalPersons) {
                    return $value['amount'];
                }
            }
        } else {
            foreach ($comedian->percent_value as $key => $value) {
                if ($value['min_person'] == 0 && $value['max_person'] == null) {
                    return $event_total * ($value['percentage'] / 100);
                } elseif ($value['max_person'] != null && $value['min_person'] <= $totalPersons && $value['max_person'] >= $totalPersons) {
                    return $event_total * ($value['percentage'] / 100);
                } elseif ($value['max_person'] == null && $value['min_person'] <= $totalPersons) {
                    return $event_total * ($value['percentage'] / 100);
    ...
    {
        $originalEvent = Event::where('id', $event_id)->with('tickets', 'venues')->without('bookings', 'user')->first();
        $newEvent = $originalEvent->replicate();
        $newEvent->status = 1;
        $title = $originalEvent->title . ' duplicate ' . rand(0, 999);
        $newEvent->slug = self::generateSlug($title);
        $newEvent->title = $title;
        $newEvent->publish = 0;
        $newEvent->eventbrite_id = null;
        $newEvent->save();

        if ($originalEvent->repetitive == 1) {
            $originalEventSchedules = Schedule::where('event_id', $originalEvent->id)->get();
            foreach ($originalEventSchedules as $originalSchedule) {
                $newSchedule = $originalSchedule->replicate();
                $newSchedule->event_id = $newEvent->id;
                $newSchedule->save();
            }

            $originalEventServerSideDates = DB::table('serverside_dates')->where('event_id', $originalEvent->id)->get();

            foreach ($originalEventServerSideDates as $originalServerSideDate) {
                $newServerSideDate = (array) $originalServerSideDate; // Convert to array to replicate
                unset($newServerSideDate['id']); // Remove the ID to generate a new one
                $newServerSideDate['event_id'] = $newEvent->id;
                DB::table('serverside_dates')->insert($newServerSideDate);
            }
        }
        $originalEvent->tickets->each(function ($ticket) use ($newEvent) {
            $newTicket = $ticket->replicate();
            $newTicket->event_id = $newEvent->id;
            $newTicket->save();
        });
        $originalEvent->comedian->each(function ($com) use ($newEvent) {
            $newComedian = $com->replicate();
            $newComedian->event_id = $newEvent->id;
            $newComedian->save();
        });
        foreach ($originalEvent->venues as $key => $value) {
            $newEvent->venues()->sync($value->id);
        }

        return response([
            'message' => "Event duplicated Successfully",
            'new_event_id' => $newEvent->id,
            'new_event_slug' => $newEvent->slug,
        ], Response::HTTP_OK);
    }
    public function index($view = 'eventsaaspro::myevents.index', $extra = [])
    {
    ...
    protected function is_admin(Request $request)
    {
        ...
        if (Auth::user()->hasRole('admin')) {
            $request->validate([
                ...
                'organiser_id.*' => __('eventsaaspro-pro::em.organiser') . ' ' . __('eventsaaspro-pro::em.required'),
            ]);
     ...
     * @return array
     */
    public function form($slug = null, $view = 'eventsaaspro::events.form', $extra = [])
    {
        ...
        if ($slug) {
            ...
            if (!Auth::user()->hasRole('admin') && Auth::id() != $event->user_id) {
                return redirect()->route('eventsaaspro.events_index');
            }
        ...
        return EventSaaSPro::view($view, compact('event', 'organisers', 'organiser_id', 'extra', 'selected_organiser'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'category_id.*' => __('eventsaaspro-pro::em.category') . ' ' . __('eventsaaspro-pro::em.required'),
        ]);
        ...
        if (empty($event)) {
            return error(__('eventsaaspro-pro::em.event_not_created'), Response::HTTP_BAD_REQUEST);
        }

        // ====================== Notification ======================
        //send notification after bookings
        $msg[] = __('eventsaaspro-pro::em.event') . ' - ' . $event->title;
        $extra_lines = $msg;

        $mail['mail_subject'] = __('eventsaaspro-pro::em.event_created');
        $mail['mail_message'] = __('eventsaaspro-pro::em.event_info');
        $mail['action_title'] = __('eventsaaspro-pro::em.manage_events');
        $mail['action_url'] = route('eventsaaspro.myevents_index');
        $mail['n_type'] = "events";
        ...
        if (empty($request->event_id)) {
            ...
            return response()->json(['status' => true, 'id' => $event->id, 'slug' => $event->slug, 'organiser_id' => $event->user_id, 'slug' => $event->slug]);
        }
    ...
    public function store_media(Request $request)
    {
        ...
        if ($request->hasfile('images')) {
            ...
            foreach ($files as $key => $file) {
                ...
                $image_resize = Image::make($file)->encode('webp', 90)->resize(854, 480);

                $s3Path = $path . $image[$key];

                Storage::disk('s3')->put($s3Path, (string) $image_resize->encode(), 'public');

                $images[$key] = Storage::disk('s3')->url($s3Path);
            }
        }
        ...
        return response()->json(['images' => $images, 'status' => true, 'slug' => $result->slug]);
    }
    ...
    public function store_location(Request $request)
    {
        ...
        return response()->json(['status' => true, 'slug' => $event->slug, 'event' => $event]);
    }
    ...
    public function store_timing(Request $request)
    {
        ...
        if ($repetitive) {
            ...
            if (!empty($repetitive_event['update'])) {
                ...
                if ($repetitive_event['update_schedule'] == false) {
                    $msg = __('eventsaaspro-pro::em.schedules_not_updated');
        ...
        return response()->json(['slug' => $event->slug, 'status' => true]);
    }
    ...
    public function store_event_tags(Request $request)
    {
        ...
        if (!empty($request->tags_ids)) {
            ...
            $tags = to_array($tags);
            if (empty($tags)) {
                $msg = __('eventsaaspro-pro::em.tag') . ' ' . __('eventsaaspro-pro::em.not_found');
        ...
        $msg = __('eventsaaspro-pro::em.event_save_success');
        return response()->json(['slug' => $check_event->slug, 'status' => true, 'message' => $msg]);
    ...
    public function store_seo(Request $request)
    {
        ...
        return response()->json(['slug' => $event->slug, 'status' => true, 'event' => $event]);
    }
    ...
    public function event_publish(Request $request)
    {
        ...
        if (count($is_publishable) < 5) {
            return error(__('eventsaaspro-pro::em.please_complete_steps'), Response::HTTP_BAD_REQUEST);
        }
    ...
    public function delete_event($slug = null)
    {
        ...
        if (Auth::check() && !Auth::user()->hasRole('admin')) {
            return redirect()->route('eventsaaspro.events');
        }
        ...
        $msg = __('eventsaaspro-pro::em.event_deleted');
    ...
    public function export_attendees($slug)
    {
        ...
        foreach ($bookings as $key => $item) {
            $bookings[$key]['event_repetitive'] = $item['event_repetitive'] ? __('eventsaaspro-pro::em.yes') : __('eventsaaspro-pro::em.no');
            $bookings[$key]['is_paid'] = $item['is_paid'] ? __('eventsaaspro-pro::em.yes') : __('eventsaaspro-pro::em.no');

            if ($item['booking_cancel'] == 1) {
                $bookings[$key]['booking_cancel'] = __('eventsaaspro-pro::em.pending');
            } elseif ($item['booking_cancel'] == 2) {
                $bookings[$key]['booking_cancel'] = __('eventsaaspro-pro::em.approved');
            } elseif ($item['booking_cancel'] == 3) {
                $bookings[$key]['booking_cancel'] = __('eventsaaspro-pro::em.refunded');
            } else {
                $bookings[$key]['booking_cancel'] = __('eventsaaspro-pro::em.no_cancellation');
            }

            if ($item['status']) {
                $bookings[$key]['status'] = __('eventsaaspro-pro::em.enabled');
            } else {
                $bookings[$key]['status'] = __('eventsaaspro-pro::em.disabled');
            }
    ...
    protected function prepare_repetitive_event(Request $request)
    {
            ...
            foreach ($request->repetitive_dates as $key => $item) {
                if ($request->from_time[$key] != 'Invalid date' || $request->to_time[$key] != 'Invalid date' || !empty($item)) {
                    $request->validate([
                        ...
                        'from_time.' . $key => ['required'],
                        'to_time.' . $key => ['required'],
                    ], [
        ...
        if (($repetitive_type == 2) && !empty($request->repetitive_days)) {
            if (!empty($event->merge_schedule)) {
                $this->prepare_schedules_weekly($request, $schedules, $from_date, $repetitive_type);
            }
    ...
    protected function update_schedule(Request $request, $schedules = [])
    {
        ...
        $msg = __('eventsaaspro-pro::em.invalid') . ' ' . __('eventsaaspro-pro::em.data');
        return ['status' => false, 'error' => $msg];
    }
    ...
    protected function prepare_schedules(Request $request, array $schedules = [], array $from_date = [], $repetitive_type = null): array
    {
        ...
        foreach ($repetitive_dates as $key => $value) {
            ...
            if (!empty($client_dates)) {
                ...
                foreach ($client_dates as $key1 => $value1) {
                    ...
                    $from_time_array = explode(',', $request->from_time[$key]);
                    $to_time_array = explode(',', $request->to_time[$key]);
                    $date = serverTimezone($month . '-' . $value1 . ' ' . $from_time_array[$key1], 'Y-m-d H:i:s', 'Y-m-d H:i:s');
                    $from_time[\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $date)->format('Y-m-d')][] = serverTimezone($from_time_array[$key], 'H:i:s', 'H:i:s');

                    $to_time[\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $date)->format('Y-m-d')][] = serverTimezone($to_time_array[$key], 'H:i:s', 'H:i:s');
                ...
                foreach ($schedules_dates as $key1 => $d) {
                    ...
                    $schedules[$key1]['from_time'] = ($request->from_time[$count] == 'Invalid date') ? null : $request->from_time[$count];
                    $schedules[$key1]['to_time'] = ($request->to_time[$count] == 'Invalid date') ? null : $request->to_time[$count];
                    // generate from start_date and end_date
            ...
            $count++;
            $temp_schedules_dates = array_merge($temp_schedules_dates, $schedules_dates);
        ...
        $this->save_server_dates($request, $temp_schedules_dates, $repetitive_type);
 ...
 */
    ...
    protected function prepare_schedules_weekly(Request $request, array $schedules = [], array $from_date = [], $repetitive_type = null): array
    {
        // current date time
        $current_date_time = Carbon::now()->toDateTimeString();
        ...
        $repetitive_dates = $request->repetitive_days;
        ...
        foreach ($repetitive_dates as $key => $value) {
            ...
            if (!empty($value)) {
                $client_dates = explode(',', $value);
                foreach ($client_dates as $index1 => $item) {
                    $client_week_days[\Carbon\Carbon::createFromFormat('Y-m-d', \Carbon\Carbon::createFromFormat('Y-m-d', $from_date[$key])->format('Y-m') . '-' . $item)->format('Y-m-d')] = \Carbon\Carbon::createFromFormat('Y-m-d', \Carbon\Carbon::createFromFormat('Y-m-d', $from_date[$key])->format('Y-m') . '-' . $item)->format('l');
            ...
            for ($i = 1; $i <= $count_days; $i++) {
                ...
                $week_days[\Carbon\Carbon::createFromFormat('Y-m-d', \Carbon\Carbon::createFromFormat('Y-m-d', $from_date[$key])->format('Y-m') . '-' . $dates[$i - 1])->format('Y-m-d')] = \Carbon\Carbon::createFromFormat('Y-m-d', \Carbon\Carbon::createFromFormat('Y-m-d', $from_date[$key])->format('Y-m') . '-' . $dates[$i - 1])->format('l');
            }
            ...
            if (!empty($client_dates)) {
                ...
                foreach ($client_week_days as $index2 => $item2) {
                    foreach ($week_days as $index3 => $item3) {
                        if ($item2 == $item3) {
                            $client_dates[] = \Carbon\Carbon::createFromFormat('Y-m-d', $index3)->format('d');
                        }
                    }
                }
