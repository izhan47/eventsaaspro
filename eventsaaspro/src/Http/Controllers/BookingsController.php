...
use Eventsaaspro\Services\PaypalExpress;
use Eventsaaspro\Services\StripeService;
...
class BookingsController extends Controller
{
    ...
    protected function is_admin_organiser(Request $request)
    {
        ...
        if (Auth::check()) {
            ...
            if (empty($event)) {
                return ['status' => false, 'error' => __('eventsaaspro-pro::em.event') . ' ' . __('eventsaaspro-pro::em.not_found')];
            }
            ...
            if (Auth::user()->hasRole('admin') || Auth::user()->hasRole('organiser')) {
                ...
                $request->validate([
                    ...
                    'customer_id.*' => __('eventsaaspro-pro::em.customer') . ' ' . __('eventsaaspro-pro::em.required'),
                ]);
    ...
    protected function availability_validation($params = [])
    {
        ...
        foreach ($selected_tickets as $key => $value) {
            ...
            if ($value['quantity'] > (!empty($ticket->customer_limit) ? $ticket->customer_limit : setting('booking.max_ticket_qty'))) {
                $msg = __('eventsaaspro-pro::em.max_ticket_qty');
            ...
            foreach ($selected_tickets as $k => $selected_ticket) {
                if ($ticket->id == $selected_ticket['ticket_id']) {
                    ...
                    if ($selected_ticket['quantity'] > $ticket->quantity) {
                        return ['status' => false, 'error' => $ticket->title . ' ' . __('eventsaaspro-pro::em.vacant') . ' - ' . $ticket->quantity];
                    }
                    ...
                    foreach ($bookings as $k2 => $booking) {
                        ...
                        if ($booking->event_start_date == $booking_date && $booking->ticket_id == $ticket->id) {
                            ...
                            if ($selected_ticket['quantity'] > $available) {
                                return ['status' => false, 'error' => $ticket->title . ' ' . __('eventsaaspro-pro::em.vacant') . ' - ' . $available];
                            }
    ...
    protected function general_validation(Request $request)
    {
        ...
        if ((int) $request->payment_method == 2) {
            if (Auth::user()->role->name != 'organiser' && Auth::user()->role->name != 'admin') {
                $request->validate([
        ...
        if (empty($ticket_ids)) {
            return ['status' => false, 'error' => __('eventsaaspro-pro::em.select_a_ticket')];
        }
        ...
        if ($tickets->isEmpty()) {
            return ['status' => false, 'error' => __('eventsaaspro-pro::em.tickets') . ' ' . __('eventsaaspro-pro::em.not_found')];
        }
    ...
    protected function time_validation($params = [])
    {
        ...
        if ($end_date < $today_date) {
            return ['status' => false, 'error' => __('eventsaaspro-pro::em.event') . ' ' . __('eventsaaspro-pro::em.ended')];
        }
        ...
        if ($hours < $default_prebook_time) {
            return ['status' => false, 'error' => __('eventsaaspro-pro::em.bookings_over')];
        }
    ...
    public function book_tickets(Request $request)
    {
        ...
        if (!$status) {
            return response([
                'status' => false,
                'url' => route('eventsaaspro.events_index'),
                'message' => __('eventsaaspro-pro::em.organiser_note_5'),
            ], Response::HTTP_OK);
        }
        ...
        if ($is_direct_checkout) {
            ...
            $msg = __('eventsaaspro-pro::em.booking_success');
            session()->flash('status', $msg);

            // if customer then redirect to mybookings
            $url = route('eventsaaspro.mybookings_index');

            if (Auth::user()->hasRole('organiser')) {
                $url = route('eventsaaspro.obookings_index');
            }
    ...
    public function paypal_callback(Request $request)
    {
        ...
        if (empty($request->paymentId)) {
            $msg = __('eventsaaspro-pro::em.booking') . ' ' . __('eventsaaspro-pro::em.failed');
            // if customer then redirect to mybookings
            $url = route('eventsaaspro.mybookings_index');
            if (Auth::user()->hasRole('organiser')) {
                $url = route('eventsaaspro.obookings_index');
    ...
    protected function finish_checkout($flag = [])
    {
        ...
        // redirect no matter what so that it never turns back
        $msg = $flag['message'];
        // $msg = __('eventsaaspro-pro::em.payment') . ' ' . __('eventsaaspro-pro::em.failed');

        if (\Request::wantsJson()) {
            return response(['status' => false, 'message' => $msg], Response::HTTP_OK);
        }
    ...
    public function login_first()
    {
        ...
        return redirect()->route('eventsaaspro.login');
    }
    ...
    public function signup_first()
    {
        ...
        return redirect()->route('eventsaaspro.register_show');
    }
    ...
    public function customer_limit($ticket = null, $selected_ticket = null, $booking_date = null)
    {
        ...
        if (!empty($ticket->customer_limit)) {
            ...
            $msg = __('eventsaaspro-pro::em.already_booked');
    ...
    protected function multiple_payment_method($order = [], $booking = [])
    {
        ...
        if ($payment_method == 2) {
            if (empty(setting('apps.stripe_public_key')) || empty(setting('apps.stripe_secret_key')) || empty(setting('apps.stripe_comic_book_fee_account_id'))) {
                return response()->json(['status' => false, 'url' => $url, 'message' => $msg]);
            }

            return $this->finish_checkout($this->stripe->stripeCharge($order, $currency, $booking));
        }
    }
