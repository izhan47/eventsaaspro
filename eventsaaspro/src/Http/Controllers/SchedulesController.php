...
class SchedulesController extends Controller
{
    ...
    public function schedules(Request $request)
    {
        ...
        if (empty($schedules)) {
            return error(__('eventsaaspro-pro::em.schedule') . ' ' . __('eventsaaspro-pro::em.not_found'), Response::HTTP_BAD_REQUEST);
        }
        if (sizeof($schedules) > 0) {
            foreach ($schedules as $key => $value) {
                $fromTime = [];
                $fromTime[] = $value['from_time'];
                $schedules[$key]['from_time'] = $fromTime;

                $toTime = [];
                $toTime[] = $value['to_time'];
                $schedules[$key]['to_time'] = $toTime;
            }
        }
    ...
    protected function prepare_schedules(Request $request, array $schedules = []): array
    {
        ...
        foreach ($schedules as $key => $value) {
            ...
            if (!empty($repetitive_dates)) {
                ...
                $value['from_time'] = explode(',', $value['from_time']);
                $value['to_time'] = explode(',', $value['to_time']);
            }
            ...
            if (!empty($server_dates)) {
                ...
                foreach ($server_dates as $key1 => $value1) {
                    ...
                    // dd($value['from_time'],$value['to_time']);

                    $date = userTimezone($value1, 'Y-m-d H:i:s', 'Y-m-d H:i:s');
                    $from_time[\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $date)->format('Y-m-d')][] = userTimezone($value['from_time'][$key1], 'H:i:s', 'H:i:s');

                    $to_time[\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $date)->format('Y-m-d')][] = userTimezone($value['to_time'][$key1], 'H:i:s', 'H:i:s');

                    $schedules_dates[\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $date)->format('Y-m-d')][] = $date;
                ...
                foreach ($schedules_dates as $key1 => $d) {
                    ...
                    $user_schedules[$key1]['from_time'] = ($value['from_time'] == null) ? null : $from_time[$key1];
                    $user_schedules[$key1]['to_time'] = ($value['to_time'] == null) ? null : $to_time[$key1];
