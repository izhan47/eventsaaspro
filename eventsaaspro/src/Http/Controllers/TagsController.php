...
class TagsController extends Controller
{
     ...
     * @return array
     */
    public function form($id = null, $view = 'eventsaaspro::tags.form', $extra = [])
    {
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $image = time() . rand(1, 988) . '.' . 'webp';

            $image_resize = Image::make($file)->encode('webp', 90)->resize(512, 512);

            Storage::disk('s3')->put($path . $image, (string)$image_resize, 'public');
    ...
    public function delete(Request $request)
    {
        ...
        if($total_tags <= 1)
        {
            return error(__('eventsaaspro-pro::em.tag').' '.__('eventsaaspro-pro::em.required'), Response::HTTP_BAD_REQUEST );
        }
