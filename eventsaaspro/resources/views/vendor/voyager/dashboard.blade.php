{{-- Organizer dashboard --}}
@extends($isOrgDash ? 'eventsaaspro::layouts.app' : 'voyager::master')

@if ($isOrgDash)
    @section('title')
        @lang('eventsaaspro-pro::em.dashboard')
    @endsection
@else
    @section('page_header')
                    ...
                    <div class="dropdown pull-right custom-dashboard-dropdown">
                        ...
                        <ul class="dropdown-menu dropdown-menu-center" aria-labelledby="dropdownMenu1">
                            @foreach ($total_notifications as $notification)
                                <li>
                                    <a href="{{ route('eventsaaspro.notify_read', ['n_type' => $notification->n_type]) }}">
            ...
            <div class="col-md-12">
                        ...
                        <div class="col-md-2">
                            <div class="box">
                                ...
                                <div class="info">
                                    ...
                                    <p>{{ __('voyager::generic.Customers') }} </p>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-2">
                            <div class="box">
                                ...
                                <div class="info">
                                    ...
                                    <p>{{ __('voyager::generic.Organisers') }}</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-2">
                        ...
                        <div class="col-md-2">
                            <div class="box">
                                ...
                                <div class="info">
                                    ...
                                    <p>{{ __('voyager::generic.Bookings') }}</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="box">
                                ...
                                <div class="info">
                                    <h3>{{ setting('regional.currency_default') . ' ' . $total_revenue }}</h3>
                    ...
                    <div class="panel panel-bordered">
                        <div class="panel-body">
                                    ...
                                    <form>
                                        ...
                                        <div class="form-group">
                                            <label class="custom-control"
                                                id="event_id_label">{{ __('eventsaaspro-pro::em.select') . ' ' . __('eventsaaspro-pro::em.event') }}</label>
                                            <select class="form-control" name="event_id" id="event_id">
                                                <option>{{ __('voyager::generic.all') }}
                                                    {{ __('voyager::generic.Events') }}</option>
                                        ...
                                        <div class="form-group">
                                            <label class="custom-control"
                                                id="ticket_lable">{{ __('eventsaaspro-pro::em.select') . ' ' . __('eventsaaspro-pro::em.ticket') }}</label>
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <form
                                        action="{{ $isOrgDash ? route('eventsaaspro.export_sales_report') : route('voyager.export_sales_report') }}"
                                        method="POST">
                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                        ...
                                        <button type="submit" id="export_button" class="btn lgx-btn btn-block"><i
                                            ...
                                            {{ __('eventsaaspro-pro::em.export_sales_report') }} CSV </button>
                                    </form>
                                </div>
                            </div>
                                    ...
                                    <table class="table table-striped table-hover table-condensed" id="sales_report">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>{{ __('voyager::generic.Booking') }} {{ __('voyager::generic.Date') }}
                                                </th>
                ...
                <div class="panel panel-bordered">
                    <div class="panel-body">
                        <div class="row">
                            <div class="col-md-12">
                                <h3>{{ __('eventsaaspro-pro::em.event_tickets_statistics') }}</h3>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-6">
                                <form>
                                    ...
                                    <div class="form-group">
                                        <select class="form-control" name="event_id_ticket_stats"
                                            ...
                                            <option>{{ __('voyager::generic.all') }} {{ __('voyager::generic.Events') }}
                                            </option>
                                            @foreach ($events as $key => $value)
                                                <option value="{{ $value['id'] }}"> {{ $value['title'] }} </option>
                                            @endforeach
                                ...
                                <table class="table table-striped table-hover table-condensed" id="event_total">
                                    <thead>
                                        <tr>
                                            <th>{{ __('voyager::generic.Event') }}</th>
                                            <th>{{ __('voyager::generic.Ticket') }}</th>
                                            <th>{{ __('eventsaaspro-pro::em.quantity') }}</th>
...
@section('javascript')
    ...
    <script>
        var url = {!! json_encode($isOrgDash ? route('eventsaaspro.sales_report') : route('voyager.sales_report')) !!}
        ...
        function sales_report(ticket_id = null) {
            ...
            $('#sales_report').DataTable({
                ...
                ajax: {
                    url: {!! json_encode($isOrgDash ? route('eventsaaspro.sales_report') : route('voyager.sales_report')) !!},
                    type: 'POST',
                    data: {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        name: 'checked_in',
                        render: function(data, type, row) {
                            return row.checked_in > 0 ? "@lang('eventsaaspro-pro::em.yes')" + " (" + row.checked_in + "/" +
                                row.quantity + ")" : "@lang('eventsaaspro-pro::em.no')"
                        }
                    },
                    {
                        data: 'ticket_price',
                        name: 'ticket_price',
                        render: function(data, type, row) {
                            return row.currency + ' ' + row.ticket_price + '<p>(' + row.ticket_title +
                    ...
                    },
                    {
                        ...
                        name: 'net_price',
                        render: function(data, type, row) {
                            return row.currency + ' ' + row.net_price
                        }
                    },
                    {
                        data: 'name',
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            return row.currency + ' ' + (row.organiser_earning ? row.organiser_earning : 0);
                        }
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            return row.currency + ' ' + (row.admin_commission ? row.admin_commission : 0);
                        }
                    },
                    {
                        data: 'admin_tax',
                        name: 'admin_tax',
                        render: function(data, type, row) {
                            return row.currency + ' ' + (row.admin_tax ? row.admin_tax : 0);
                        }
                    },

                    {
                        ...
                        render: function(data, type, row) {
                            return (row.transferred <= 0 && row.organiser_earning > 0) ?
                                "@lang('eventsaaspro-pro::em.pending')" : "@lang('eventsaaspro-pro::em.transferred')";
                        }
                    },
                ],
                ...
                footerCallback: function(row, data, start, end, display) {
                    ...
                    var msg = "@lang('eventsaaspro-pro::em.total')";
                    $(api.column(0).footer()).html(msg);
                    $(api.column(6).footer()).html(currency + ' ' + total_ticket_price.toFixed(2));
                    ...
                    // Update footer by showing the total with the reference of the column index
                    $(api.column(7).footer()).html(currency + ' ' + customer_paid.toFixed(2));
                    ...
                    // Update footer by showing the total with the reference of the column index
                    $(api.column(9).footer()).html(currency + ' ' + organiser_earning.toFixed(2));
                    ...
                    // Update footer by showing the total with the reference of the column index
                    $(api.column(10).footer()).html(currency + ' ' + admin_commission.toFixed(2));
                    ...
                    // Update footer by showing the total with the reference of the column index
                    $(api.column(11).footer()).html(currency + ' ' + admin_tax.toFixed(2));
        ...
        function events_total_by_sales_price() {
            ...
            $('#event_total').DataTable({
                ...
                ajax: {
                    url: {!! json_encode(
                        $isOrgDash ? route('eventsaaspro.event_total_by_sales_price') : route('voyager.event_total_by_sales_price'),
                    ) !!},
                    type: 'POST',
                    data: {
                        _token: {!! json_encode(csrf_token()) !!},
        ...
        function getEvent() {
            var url = {!! json_encode($isOrgDash ? route('eventsaaspro.get_event') : route('voyager.get_event')) !!}
            ...
            $.ajax(url, {
                ...
                success: function(res, status, xhr) { // success callback function
                    ...
                    if (res.status) {
                        ...
                        mySelect.append(
                            $('<option></option>').val({!! json_encode(__('eventsaaspro-pro::em.all')) !!}).html(
                                {!! json_encode(__('eventsaaspro-pro::em.all')) !!})
                        );
