...
@section('title')
    @if (empty($event))
        @lang('eventsaaspro-pro::em.create_event')
    @else
        @lang('eventsaaspro-pro::em.update_event')
    @endif
@endsection
    ...
    <div class="row">
        <div class="col-xl-12 col-md-12 col-12">
            <div class="card shadow-sm border-0">
                <div class="card-header d-flex align-items-start justify-content-between p-4 bg-white border-bottom-0" style="min-height: 140px;">
                    <div>
                        <h1 class="mb-0 fw-bold h2" style="max-width: 100%;">
                            @if (empty($event))
                                @lang('eventsaaspro-pro::em.create_event')
                            @else
                                @lang('eventsaaspro-pro::em.update_event') - {{ $event->title }}
                            @endif
                        </h1>
                    </div>
                    <div class="d-flex align-items-start">
                        <a class="btn btn-secondary btn-block" href="{{ route('eventsaaspro.myevents_index') }}"><span><i class="fas fa-xmark"></i> @lang('eventsaaspro-pro::em.cancel')</span></a>
                    </div>
                </div>
