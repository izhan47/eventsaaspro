    ...
    <div>
        <div class="card shadow-sm border-0">
                        ...
                        <div class="card bg-info mb-4 border-0 shadow-sm">
                            <div class="card-body p-4">
                                ...
                                <div class="d-flex align-items-center justify-content-between mt-4">
                                    <h2 class="fw-bold mb-0 fs-1 text-white">{{ currency }} {{ total_earning.customer_paid_total }}</h2>
                    ...
                    <div class="col-lg-4 col-md-12 col-12">
                        <div class="card bg-dark mb-4 border-0 shadow-sm">
                            <div class="card-body p-4">
                                ...
                                <div class="d-flex align-items-center justify-content-between mt-4">
                                    <h2 class="fw-bold mb-0 fs-1 text-white">{{ currency }} {{ total_earning.admin_commission_total }}</h2>
                    ...
                    <div class="col-lg-4 col-md-12 col-12">
                        <div class="card bg-success mb-4 border-0 shadow-sm">
                            <div class="card-body p-4">
                                ...
                                <div class="d-flex align-items-center justify-content-between mt-4">
                                    <h2 class="fw-bold mb-0 fs-1 text-white">{{ currency }} {{ total_earning.organiser_earning_total }}</h2>
                ...
                <table class="table text-wrap">
                    <thead class="table-light text-nowrap">
                        <tr>
                            ...
                            <th class="border-top-0 border-bottom-0">Comedian Commission</th>
                            <th class="border-top-0 border-bottom-0">Event {{ trans('em.profit') }}     </th>
                        ...
                        <tr v-for="(earning, index) in event_earning" :key="index" >
                            <td :data-title="trans('em.event')">
                             ...
                             <td class="align-middle" :data-title="trans('em.status')">
                                 <span class="badge bg-success" v-if="earning.event_status == 1">{{ trans('em.enabled')  }}</span>
                                <span class="badge bg-success" v-else-if="earning.event_status == 2">{{ trans('em.completed')  }}</span>
                                <span class="badge bg-danger" v-else>{{ trans('em.disabled') }}</span>
                            </td>
                            <td class="align-middle" :data-title="trans('em.bookings')">
                                {{ currency }} {{ earning.customer_paid_total }}
                            </td>
                            <!-- <td class="align-middle" :data-title="trans('em.commission')">
                                {{ currency }} {{ earning.admin_commission_total }}
                            </td> -->
                            <td class="align-middle text-danger" :data-title="trans('em.commission')">
                                <i class="fas fa-sack-dollar"></i> {{ currency }} {{ earning.comedian_total_commission.toFixed(2) }}
                            </td>
                            <td class="align-middle text-success" :data-title="trans('em.profit')">
                                <template v-if="earning.comedian_total_commission == 0.00">
                                    <i class="fas fa-sack-dollar"></i> {{ currency }} 0
                                </template>
                                <template v-else >
                                    <i class="fas fa-sack-dollar"></i> {{ currency }} {{ (earning.organiser_earning_total - earning.comedian_total_commission).toFixed(2) }}
                                </template>
                            </td>
            ...
            <div class="px-4 pb-4"  v-if="event_earning.length > 0">
                <pagination-component v-if="pagination.last_page > 1" :pagination="pagination" :offset="pagination.total" :path="'/dashboard/myearning'" @paginate="eventEarning()">
...
export default {
    ...
    methods:{
        ...
        // get all events
        getMyEvents() {
            axios.get(route('eventsaaspro.all_myevents'))
            .then(res => {
        ...
        eventEarning() {
            ...
            axios.get(route('eventsaaspro.organiser_event_earning')+'?page='+this.current_page+'&event_id='+this.event_id+'&start_date='
        ...
        //net total
        totalEarning() {
             axios.get(route('eventsaaspro.organiser_total_earning'))
            .then(res => {
