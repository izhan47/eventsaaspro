                    ...
                    <form ref="form" @submit.prevent="cropThumbnailPoster" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row mb-3">
                            <div class="col-sm-12 d-flex align-items-center justify-content-end">
                                <button type="submit" class="btn btn-primary btn-lg"><i class="fas fa-sd-card"></i> {{ trans('em.saveNext') }}</button>
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        ...
        formSubmit(event) {
            ...
            let post_url = route('eventsaaspro.myevents_store_media');
            ...
            .then(res => {
                ...
                if(res.data.status)
                {
                    ...
                    if(res.data.slug){
                        setTimeout(function() {
                            window.location = route('eventsaaspro.myevents_form',[res.data.slug])+'#/comedians';
                        }, 2000);
                    }
                }
            })
        ...
        editMedia(){
            ...
            {
                this.thumbnail_preview         = this.event.thumbnail ? (this.event.thumbnail) : null;
                this.poster_preview            = this.event.poster ? (this.event.poster) : null;
        ...
        deleteGalleryImages(image = null){
               this.showConfirm(trans('em.delete')).then((res) => {
                if(res) {
                    // prepare form data for post request
                    let post_url = route('eventsaaspro.delete_image');
                    let post_data = {
