        ...
        <form ref="form" @submit.prevent="validateForm" method="POST" enctype="multipart/form-data"  class="lgx-contactform">
            ...
            <div class="row mb-3">
                <div class="col-md-12 d-flex align-items-center justify-content-end">
                    <button type="submit" class="btn btn-primary mt-2 btn-lg"><i class="fas fa-sd-card"></i> {{ trans('em.saveNext') }}</button>
                </div>
            </div>
            ...
            >
                ...
                <div class="col-md-12">
                    <RepetitiveScheduleComponent
                        :months="calculate_months"
                        :schedule_repetitive_dates="schedule_repetitive_dates"
...
import RepetitiveScheduleComponent from './RepetitiveSchedule.vue';
...
export default {
    ...
    methods: {
        ...
        getSchedule()
        {
            let post_url = route('eventsaaspro.schedules');
            let post_data = {
            ...
            .then(res => {
                ...
                this.r_type      = this.event.repetitive_type;
                ...
                if(this.schedules.length > 0){
                    ...
                    this.schedule_repetitive_dates = [];
                    this.schedules.forEach((value, key) => {
                        this.schedule_repetitive_dates[key] = JSON.parse(value["repetitive_dates"]);
                    })

                    this.schedule_from_time = [];
                    this.schedules.forEach((value, key) => {
                        this.schedule_from_time[key] = value["from_time"];
                    })

                    this.schedule_to_time = [];
                    this.schedules.forEach((value, key) => {
                        this.schedule_to_time[key] = value["to_time"];
                    })
                }
        ...
        convertToServerDate(data, type) {
            ...
            if (type === 'dates') {
                data.forEach((item,index) => {
                    v_repetitive_dates[index] = '';
                    item.forEach((item2, innerIndex) => {
                        v_repetitive_dates[index] += item2.value.toString()
        ...
        formSubmit(event) {
            ...
            {
                v_repetitive_dates  = this.convertToServerDate(this.v_repetitive_dates, 'dates');

            }
            // v_repetitive_days
            if(this.v_repetitive_days.length > 0)
                v_repetitive_days   = this.convertToServerDate(this.v_repetitive_days, 'days');

            if(this.v_from_time.length > 0)
            {
                // this.v_from_time.forEach((item) => {
                //     item.forEach(item2 => {
                //         v_from_time.push(item2)
                //     })
                // })


                this.v_from_time.forEach((item,index) => {
                    v_from_time[index] = '';
                    item.forEach((item2, innerIndex) => {
                        v_from_time[index] += item2
            ...
            if(this.v_to_time.length > 0)
            {
                // this.v_to_time.forEach((item) => {
                //     item.forEach(item2 => {
                //         v_to_time.push(item2)
                //     })
                // })
                this.v_to_time.forEach((item,index) => {
                    v_to_time[index] = '';
                    item.forEach((item2, innerIndex) => {
                        v_to_time[index] += item2
            ...
            let post_url = route('eventsaaspro.myevents_store_timing');
            ...
            .then(res => {
                ...
                if(res.data.status)
                {
                    ...
                    if(res.data.slug)
                    {
                        ...
                        setTimeout(function() {
                            window.location = route('eventsaaspro.myevents_form',[res.data.slug])+'#/tickets';
                        }, 1000);
                    }
                    // reload page
                    // if(res.data.slug){
