<template>
    <div>
        ...
        <div class="bg-light card shadow-sm mt-3">
            <!-- Card header -->
            <div class="card-header p-4 bg-white">
                <h3 class="mb-0">Event Complete</h3>
                <p class="mb-0">You can change event status by clicking at button below</p>
            </div>
            <!-- Card body -->
            <div class="card-body p-4">
                <div v-if="event.status == 2">
                    <span class="text-danger h4">Enable Event</span>
                    <p class="text-danger">You can change status of event to enable by clicking at button.</p>
                </div>

                <button type="button" class="btn btn-outline-success btn-lg"
                    :class="{ 'btn-outline-danger': (event.status != 1), 'btn-outline--success': (event.status == 1) }"
                    @click="changeEventStatus(event.status === 1 ? 2 : 1)"
                >
                    <i v-if="event.status === 1" class="fas fa-eye"></i>
                    <i v-if="event.status === 2" class="fas fa-eye-slash"></i>
                    {{ event.status == 1 ? 'Complete Event' : 'Enable Event'}}
                </button>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        formSubmit(event) {
            ...
            // prepare form data for post request
            let post_url    = route('eventsaaspro.myevents_store_event_tags');
        ...
        getSelectedtags() {
            let $this = this;
            axios.post(route('eventsaaspro.selected_tags'),{
                event_id      : this.event_id,
                organiser_id  : this.organiser_id,
            })
            .then(res => {
        ...
        // publish event
        publishEvent(){
            axios.post(route('eventsaaspro.publish_myevent'),{
                event_id        : this.event_id,
                organiser_id    : this.organiser_id,
            })
            .then(res => {
        ...
        changeEventStatus(status) {
            axios.get(`/dashboard/myevents/api/complete_event/${this.event_id}/${status}`)
            .then((response) => {
        ...
        searchTags: _.debounce(function(search) {
            ...
            let post_url    = route('eventsaaspro.search_tags');
