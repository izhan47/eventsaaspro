<template>
    <div>
        <div>
            ...
            <br/>
            <small>Want to clone tickets from an existing event <span @click="openCloneTicketModal = true" class="text-primary pointer">click here</span></small>
        </div>
        ...
        <CloneTickets :isOpen="openCloneTicketModal" />
        <div class="row mb-3">
        </div>
        <div class="row">
                ...
                <table class="table text-wrap">
                    ...
                    <tbody>
                        ...
                        >
                            ...
                            <td :data-title="trans('em.price')">{{ currency+' '+ ticket.price }}</td>
...
import CloneTickets from './CloneTickets.vue';
    ...
    methods: {
        ...
        deleteTicket(ticket_id){
            this.showConfirm(trans('em.ticket_delete_ask')).then((res) => {
                if(res) {
                    axios.post(route('eventsaaspro.tickets_delete'), {
                        ticket_id       : ticket_id,
        ...
        // get all tickets
        getTickets() {
            axios.post(route('eventsaaspro.tickets'),{
                'event_id'      : this.event_id,
                'organiser_id'  : this.organiser_id,
            })
            .then(res => {
        ...
        getTaxes(){
            axios.get(route('eventsaaspro.tickets_taxes'),{
                'event_id'      : this.event_id,
                'organiser_id'  : this.organiser_id,
            })
            .then(res => {
