            ...
            <div class="card-header d-flex justify-content-between flex-wrap p-4 bg-white border-bottom-0">
                <div class="d-flex flex-column">
                    ...
                    <div class="d-flex">
                        ...
                        <div class="me-2">
                            <input type="text" class="form-control" v-model="search" placeholder="Enter Title or Email or Order #" />
                        </div>
                    </div>
                </div>
            </div>
                ...
                <table class="table text-wrap">
                    ...
                    <tbody>
                        <tr v-for="(booking, index) in bookings" :key="index" >
                            <td :data-title="trans('em.event')">
                                <div class="d-flex align-items-center">
                                    <a :href="eventSlug(booking.event_slug)">
                                        <img :src="booking.event_thumbnail" :alt="booking.event_title" class="rounded img-4by3-md ">
                                    </a>
                                    <div class="ms-3 lh-1">
                                        <h5 class="mb-1">
                            ...
                            <td class="align-middle" :data-title="trans('em.order_total')">{{ currency+' '+booking.net_price}} </td>
...
export default {
    ...
    methods:{
        ...
        getOrganiserBookings() {
            ...
            axios.get(route('eventsaaspro.obookings_organiser_bookings')+'?page='+this.current_page+'&event_id='+this.event_id+'&start_date='
                         +this.start_date+'&end_date='+this.end_date+'&search='+this.search)
            .then(res => {
        ...
        organiserViewBooking(booking_id) {
            axios.get(route('eventsaaspro.obookings_organiser_bookings_show',[booking_id]))
            .then(res => {
                if(res.data.status)
                {
        ...
        // view booking
        goto_route(id) {
            return route('eventsaaspro.obookings_organiser_bookings_show', {id:id});
        },
        ...
        downloadURL(id, order_number) {
            if(id && order_number) {
                return route('eventsaaspro.downloads_index',[id, order_number]);
            }
        },
