                            ...
                            <td :data-title="trans('em.venue')">
                                <div class="d-flex align-items-center">
                                    <a :href="venueSlug(item.slug)">
                                        <img :src="venueImage(JSON.parse(item.images)[0])" :alt="item.title" class="rounded img-4by3-md ">
                                    </a>
                                    <div class="ms-3 lh-1">
                                        <h5 class="mb-1">
...
export default {
    ...
    methods: {
        getVenues() {
            axios.get(route('eventsaaspro.myvenues.index')+'?page='+this.current_page+'&organiser_id='+this.organiser_id)
            .then(res => {
        ...
        deleteVenue(venue_id){
            ...
            this.showConfirm(trans('em.delete_venue_ask')).then((res) => {
                if(res) {
                    ...
                    axios.post(route('eventsaaspro.myvenues.destroy',[venue_id]), {
        ...
        venueSlug(slug){
            return route('eventsaaspro.venues.show',[slug]);
        },
