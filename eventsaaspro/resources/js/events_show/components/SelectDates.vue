...
<template>
    <div class="col-xs-12">
        <div  v-if="event.repetitive > 0">
            <div class="bg-white rounded-3 px-3 mb-3">
                <ul class="nav nav-lb-tab h-scroll">
                    ...
                    >
                        <template v-if="schedules[index].from_time != null && schedules[index].to_time != null">
            ...
            <div class="tab-content v-scroll mh-30 p-3" v-if="event.merge_schedule <= 0">
                <div v-for="(item, index) in calculate_months" :key="index"   class="tab-pane" :class="{'active' : index == tab_active_index}" >
                    <template v-if="schedules[index].from_time != null && schedules[index].to_time != null">
                        <div class="row" role="tablist" aria-multiselectable="true" v-if="parseInt(schedules[0].repetitive_type) === parseInt(3)">
                            <a role="button" data-toggle="collapse" aria-expanded="true" aria-controls="collapseOne">
                                ...
                                <div class="card border-0 shadow-sm img-hover mb-2 lgx-single-schedule"
                                    :class="{
                                        'expired-event': userTimezone1(item+'-'+selected_date+' '+schedules[index].to_time[0].split(',')[index1],'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD HH:mm:ss') < moment().format('YYYY-MM-DD HH:mm:ss'),
                                    }"
                                    v-for="(selected_date, index1) in newRepetitiveDates[index]"
                                    :key="index1"
                                >
                                    <div class="d-flex justify-content-between flex-wrap card-body schedule-info"
                                        @click="() => {
                                            eventStartTime = `${userTimezone1(dateToShortDate(selected_date, item), date_format.vue_date_format).format(date_format.vue_date_format)} - ${changeTimeFormat(userTimezone1(moment(schedules[index].from_date).format('YYYY-MM')+'-'+selected_date+' '+schedules[index].from_time[0].split(',')[index1], 'YYYY-MM-DD HH:mm:ss').format('HH:mm:ss'))} ${showTimezone() !== undefined ? showTimezone() : ''}`;
                                            (!(userTimezone1(item+'-'+selected_date+' '+schedules[index].to_time[0].split(',')[index1],'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD HH:mm:ss') < moment().format('YYYY-MM-DD HH:mm:ss')))
                                            ? selectDates(userTimezone1(dateToFullDate(selected_date, item),'dddd LL').format('dddd LL'), false,
                                            changeTimeFormat(userTimezone1(moment(schedules[index].from_date).format('YYYY-MM')+'-'+selected_date+' '+schedules[index].from_time[0].split(',')[index1], 'YYYY-MM-DD HH:mm:ss').format('HH:mm:ss'))
                                            , changeTimeFormat(userTimezone1(moment(schedules[index].to_date).format('YYYY-MM')+'-'+selected_date+' '+schedules[index].to_time[0].split(',')[index1], 'YYYY-MM-DD HH:mm:ss').format('HH:mm:ss')) )
                                            : null
                                        }">

                                        <h6 class="mb-0 h6 fw-bold">{{ userTimezone1(dateToShortDate(selected_date, item), date_format.vue_date_format).format(date_format.vue_date_format) }}</h6>
                                        <!-- <span class="badge bg-light text-primary">{{ changeTimeFormat(userTimezone1(moment(schedules[index].from_date).format('YYYY-MM')+'-'+selected_date+' '+schedules[index].from_time[0].split(',')[index1], 'YYYY-MM-DD HH:mm:ss').format('HH:mm:ss')) }} -->
                                        <span>{{ changeTimeFormat(userTimezone1(moment(schedules[index].from_date).format('YYYY-MM')+'-'+selected_date+' '+schedules[index].from_time[0].split(',')[index1], 'YYYY-MM-DD HH:mm:ss').format('HH:mm:ss')) }}
                                            {{ schedules[index].to_time[0].split(',')[index1] === '23:59:00' ? '' : ' - ' }}
                                            {{ schedules[index].to_time[0].split(',')[index1] === '23:59:00' ? '' :changeTimeFormat(userTimezone1(moment(schedules[index].to_date).format('YYYY-MM')+'-'+selected_date+' '+schedules[index].to_time[0].split(',')[index1], 'YYYY-MM-DD HH:mm:ss').format('HH:mm:ss')) }}
                                        ...
                                        <!-- expired -->
                                        <h4 class="mb-0 h6 badge bg-danger event-ended" v-if="userTimezone1(item+'-'+selected_date+' '+schedules[index].to_time[0].split(',')[index1],'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD HH:mm:ss') < moment().format('YYYY-MM-DD HH:mm:ss')"
                                        >
                                            <span>{{ trans('em.ended') }}</span>
                                        </h4>
                                        <span v-else class="badge bg-light text-primary"> Buy </span>

                                        <!-- out-of-stock -->
                                        <h4 class="mb-0 h6 badge bg-danger event-outofstock" v-if="
                                            !(userTimezone1(item+'-'+selected_date+' '+schedules[index].to_time[0].split(',')[index1],'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD HH:mm:ss') < moment().format('YYYY-MM-DD HH:mm:ss')) && !selected_date"
                                        >
                                        ...
                                        <h4 class="mb-0 h6 badge bg-danger event-outofstock mr-3" v-if="
                                            !(userTimezone1(item+'-'+selected_date+' '+schedules[index].to_time[0].split(',')[index1],'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD HH:mm:ss') < moment().format('YYYY-MM-DD HH:mm:ss')) && positiveInteger(selected_date)"
                                        >
        ...
        <div class="lgx-tab" v-if="event.repetitive <= 0">
            <div class="tab-content v-scroll mh-30 py-3">
                <div class="tab-pane active">
                    <div class="row" role="tablist" aria-multiselectable="true">

                        <a role="button" data-toggle="collapse" aria-expanded="true" aria-controls="collapseOne">
                            <div class="card border-0 shadow-sm img-hover lgx-single-schedule"
                                :class="{
                            ...
                            >
                                <div class="d-flex justify-content-between flex-wrap card-body schedule-info"
                                    @click="() => {
                                        eventStartTime = singleEventDateTime();
                                ...
                                >
                                    ...
                                    <!-- <span class="badge bg-light text-primary">{{ userTimezone(event.start_date+' '+event.start_time, 'YYYY-MM-DD HH:mm:ss').format(date_format.vue_time_format) }} - {{ userTimezone(event.end_date+' '+event.end_time, 'YYYY-MM-DD HH:mm:ss').format(date_format.vue_time_format) }} {{ showTimezone() }}</span> -->
                                    <span class="event-time">{{ userTimezone(event.start_date+' '+event.start_time, 'YYYY-MM-DD HH:mm:ss').format(date_format.vue_time_format) }} - {{ userTimezone(event.end_date+' '+event.end_time, 'YYYY-MM-DD HH:mm:ss').format(date_format.vue_time_format) }} {{ showTimezone() }}</span>
...
export default {
    ...
    methods: {
        ...
        getEventSchedule()
        {
            let post_url = route('eventsaaspro.event_schedule') ;
            let post_data = {
            ...
            .then(res => {
                ...
                this.schedules.forEach(function(value, key) {
                    ...
                    let dates = JSON.parse(value['repetitive_dates']);
                    dates = dates.split(',');
                    _this.newRepetitiveDates[key] = dates;
                });
        ...
        singleEventDateTime () {
            let startDate = this.convert_date_to_local_format(this.userTimezone(this.event.start_date+' '+this.event.start_time, 'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD') );
            let endDate = (this.userTimezone(this.event.start_date+' '+this.event.start_time, 'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD') <= this.userTimezone(this.event.end_date+' '+this.event.end_time, 'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD') )
            ? this.convert_date_to_local_format(this.userTimezone(this.event.end_date+' '+this.event.end_time, 'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD') )
            : this.convert_date_to_local_format(this.userTimezone(this.event.start_date+' '+this.event.start_time, 'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD'))

            let startTime = this.userTimezone(this.event.start_date+' '+this.event.start_time, 'YYYY-MM-DD HH:mm:ss').format(date_format.vue_time_format);
            let endTime = this.userTimezone(this.event.end_date+' '+this.event.end_time, 'YYYY-MM-DD HH:mm:ss').format(date_format.vue_time_format);

            return `${startDate} - ${startTime} ${this.showTimezone() !== undefined ? this.showTimezone() : ""}`
        },
    },
    mounted() {
        if(this.event.repetitive > 0)
