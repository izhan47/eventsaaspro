    ...
    <div>
        <div class="card shadow-sm border-0">
                        ...
                        <tr v-for="(item, index) in tags" v-bind:item="item" v-bind:index="index" v-bind:key="item.id">
                            <td :data-title="trans('em.name')">
                                <div class="d-flex align-items-center">
                                    <img :src="tagImage(item.image)" :alt="item.title" class="rounded img-4by3-md ">
            ...
            <div class="px-4 pb-4" v-if="tags.length > 0">
                <pagination-component v-if="pagination.last_page > 1" :pagination="pagination" :offset="pagination.total" :path="'/dashboard/mytags'" @paginate="getTags()">
...
export default {
    ...
    methods: {
        getTags() {
            axios.post(route('eventsaaspro.tags')+'?page='+this.current_page,{
        ...
        deleteTag(tag_id){
            ...
            this.showConfirm(trans('em.delete_tag_ask')).then((res) => {
                if(res) {
                    ...
                    axios.post(route('eventsaaspro.tags_delete'), {
                        tag_id : tag_id
                    })
                    .then(res => {
        ...
        tagImage(image) {
            return image !== undefined && image.includes("http") ? image : this.imageBaseUrl + image;
        },
    },
