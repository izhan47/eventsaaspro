...
export default {
    ...
    methods: {
        ...
        editTags(){
            this.imageSrc    = this.tagImageSrc(this.edit_tag.image);
        ...
        formSubmit(event) {
            ...
            // prepare form data for post request
            let post_url = route('eventsaaspro.tags_store');
        ...
        tagImageSrc(image) {
            return image.includes("http") ? image : `${window.config.s3Url}${image}`;
        }
    },
